...
tp @e[type=zombie,tag=GerudoGuard2] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard2] run summon minecraft:zombie -76 70 2003 {Tags:["GerudoGuard","GerudoGuard2","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=349..}] run kill @e[type=armor_stand,tag=GerudoGuard2]

#Moving
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=1..37}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=38..124}] run tp @s ~ ~ ~.08

execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=176..210}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=211..297}] run tp @s ~ ~ ~-.08

#Kill
execute if entity @e[type=zombie,tag=GerudoGuard2,nbt={HurtTime:10s}] run tag @s add Kill
