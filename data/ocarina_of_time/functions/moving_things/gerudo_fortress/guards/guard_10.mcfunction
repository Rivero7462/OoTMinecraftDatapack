...
tp @e[type=zombie,tag=GerudoGuard10] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard10] run summon minecraft:zombie -120 43 1948 {Tags:["GerudoGuard","GerudoGuard10","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=627..}] run kill @e[type=armor_stand,tag=GerudoGuard10]

#Moving
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=1..37}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=38..262}] run tp @s ~ ~ ~.08

execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=314..350}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=351..575}] run tp @s ~ ~ ~-.08

#Kill
execute if entity @e[type=zombie,tag=GerudoGuard10,nbt={HurtTime:10s}] run tag @s add Kill
