...
tp @e[type=zombie,tag=GerudoGuard3] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard3] run summon minecraft:zombie -64 70 2003 {Tags:["GerudoGuard","GerudoGuard3","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=349..}] run kill @e[type=armor_stand,tag=GerudoGuard3]
...
#Kill
execute if entity @e[type=zombie,tag=GerudoGuard3,nbt={HurtTime:10s}] run tag @s add Kill
