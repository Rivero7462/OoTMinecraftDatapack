...
tp @e[type=zombie,tag=GerudoGuard8] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard8] run summon minecraft:zombie -74 43 1939 {Tags:["GerudoGuard","GerudoGuard8","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=376..}] run kill @e[type=armor_stand,tag=GerudoGuard8]
...
#Kill
execute if entity @e[type=zombie,tag=GerudoGuard8,nbt={HurtTime:10s}] run tag @s add Kill
