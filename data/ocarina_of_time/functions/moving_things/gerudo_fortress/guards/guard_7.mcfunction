...
tp @e[type=zombie,tag=GerudoGuard7] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard7] run summon minecraft:zombie -69 43 1954 {Tags:["GerudoGuard","GerudoGuard7","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=367..}] run kill @e[type=armor_stand,tag=GerudoGuard7]
...
#Kill
execute if entity @e[type=zombie,tag=GerudoGuard7,nbt={HurtTime:10s}] run tag @s add Kill
