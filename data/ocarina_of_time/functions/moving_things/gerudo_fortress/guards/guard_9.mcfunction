...
tp @e[type=zombie,tag=GerudoGuard9] @s

#LifeTime
execute unless block -72 67 1988 minecraft:gold_block unless entity @s[tag=Npc] unless entity @s[tag=Kill] unless entity @s[tag=Stun] unless entity @e[type=zombie,tag=GerudoGuard9] run summon minecraft:zombie -68 43 1930 {Tags:["GerudoGuard","GerudoGuard9","Invisible","Resist","Fire"],PersistenceRequired:1b,DeathLootTable:"ocarina_of_time:entities/nothing",Silent:1,NoAI:1b}
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=376..}] run kill @e[type=armor_stand,tag=GerudoGuard9]

#Moving
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=1..37}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=38..137}] run tp @s ~-.08 ~ ~

execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=189..224}] run tp @s ~ ~ ~ ~5 ~
execute unless entity @s[tag=Kill] unless entity @s[tag=Stun] if entity @s[scores={lifetime=225..324}] run tp @s ~.08 ~ ~

#Kill
execute if entity @e[type=zombie,tag=GerudoGuard9,nbt={HurtTime:10s}] run tag @s add Kill
